...
// Скрипт для урезания количества отрисованных сообщений на странице ChatGPT
const INJECT = (limit) => `
(function() {
  const LIMIT = ${"${limit}"};
  ...
  function cut() {
    try {
      ...
      // Прячем возможные "load more"
      const more = root.querySelectorAll('button');
      for (const b of more) {
        const t = (b.textContent || '').toLowerCase();
        if (t.includes('show more') || t.includes('load more') || t.includes('показать еще')) {
  ...
  function ensureBadge() {
    if (document.getElementById('araya-badge')) return;
...
contextBridge.exposeInMainWorld('arayaAPI', {
  setLimit: (n) => {
    currentLimit = n;
    exec(INJECT(String(n)));
  }
});

ipcRenderer.on('araya:setLimit', (_e, n) => {
  currentLimit = n;
  exec(INJECT(String(n)));
});

window.addEventListener('DOMContentLoaded', () => {
  exec(INJECT(String(currentLimit)));
});
